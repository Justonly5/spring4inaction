...
public class JdbcTemplateTest {
	...
	// BeforeClass: 表示在测试方法前执行一次,且只执行一次.
	@BeforeClass
	...
	private void insert() {
		...
		jdbcTemplate.query("select count(*) as count from test", new RowCallbackHandler() {

			@Override
			public void processRow(ResultSet rs) throws SQLException {
				Assert.assertEquals(2, rs.getInt("count"));
			}
		});
		// Assert.assertEquals(2, jdbcTemplate.queryForInt("select count(*) from
		// test"));
	}

	private void delete() {
		jdbcTemplate.update("delete from test where name=?", new Object[] { "name2" });
		jdbcTemplate.query("select count(*) from test", new RowCallbackHandler() {
			@Override
			public void processRow(ResultSet rs) throws SQLException {
	...
	private void update() {
		...
		jdbcTemplate.query("select count(*) from test where name='name3'", new RowCallbackHandler() {
			@Override
			public void processRow(ResultSet rs) throws SQLException {
				Assert.assertEquals(1, rs.getInt(1));
			}
